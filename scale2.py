...
for num_l in range(10):
    for num_r in range(10):
        ...
        fig.savefig(f'./causal_data/scale/test/scale_{num_l}_{num_r}_{np.sign(tip_angle)}_{abs(tip_angle)}.png', dpi=96)
        fig.savefig(f'./causal_data/scale/train/scale_{num_l}_{num_r}_{np.sign(tip_angle)}_{abs(tip_angle)}.png', dpi=96)
        plt.clf()
